...
class AuthServiceLoginVerificationTest {
    ...
    @Test
    void login_requiresEmailVerificationOnEveryConnection_evenForTrustedIp() {
        ...
        verify(userDAO).updateLoginIpVerificationChallenge(eq(7), any(), any(), any(), eq("127.0.0.1"));
        verify(emailService).sendLoginIpVerificationEmail(eq(user), any(), any(), eq("127.0.0.1"));
    }
}
